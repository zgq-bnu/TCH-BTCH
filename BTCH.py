...
if __name__ == '__main__':
    ...
    data = pd.read_csv(r'data.csv', index_col=0).iloc[:].values.T

    all_x = data[:, :, np.newaxis, np.newaxis]

    # BTCH results
    all_x_btch = btch(all_x)

    plt.figure(figsize=(20, 5))
    plt.plot(data[0][0:100], label='A')
    plt.plot(data[1][0:100], label='B')
    plt.plot(data[2][0:100], label='C')
    plt.plot(data[3][0:100], label='D')
    plt.scatter(x=np.arange(100), y=all_x_btch[:, 0, 0][0:100], label='BTCH')
    plt.legend()
    plt.show()